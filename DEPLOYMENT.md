...
- **Milestone AF.1 ("ERP Numbering Hardening, Immutable Codes & Bulk
  Water Fleet Capacity Cleanup")** — no schema change, no migration.

  **Root cause of the production findings**: AF deliberately kept a
  manual-code path (`resolveEntityCode`: provided → validate → dedupe),
  so a well-formed "015" was accepted by design, and the PATCH routes
  validated a changed code instead of refusing it. The business decision
  in AF.1 reverses that: for converted entities, codes are
  system-generated only and immutable.

  **Policy (lib/businessCodes.ts)**: POST rejects any client-supplied
  code with "Codes are generated by the system and cannot be entered
  manually."; PATCH rejects any code change with "Code cannot be changed
  after creation." (an idempotent resend of the current value is
  tolerated); no active series → "Configure an active numbering series
  for this entity in Settings before creating records." Applies to
  Supplier, Item Group/Category/Subcategory, Item, Workshop, Maintenance
  Warehouse. Existing manual codes (e.g. "V", "015", "8788") are
  retained, readable, and immutable — no cleanup, no renumbering.

  **Preview**: `GET /api/settings/numbering-preview?entityType=…`
  returns the formatted next number using the same formatter as the
  allocator, never increments `nextNumber`, never writes a ledger row
  (verified with 20 concurrent previews). Create forms show a read-only
  "Next … code" via `components/NextCodePreview.tsx` and disable Save
  when no active series exists; the real number is allocated on save.

  **Settings hardening**: entityType from the registry only (lowercase/
  free text → 400); prefix `^[A-Z]{1,5}$`; segment uppercase/digits ≤10;
  separator ∈ {"", "-", "/"}; padding 3–10; nextNumber ≥1 and
  create-only; duplicate entityType, duplicate seriesCode, and duplicate
  effective format (prefix+segment+separator+padding) per tenant → 409.

  **Bulk Water Fleet cleanup**: "General capacity units" removed from
  the add form; "Tanker capacity (liters) — e.g. 18000 / 21000 / 28000"
  is the primary capacity; inline edit is liters-only with no
  "Units"/"units"; `capacityUnits` is never sent from the UI (column
  retained for Demo Water Co. and legacy tests). Dispatch Plan Trip now
  says "N order(s) selected" (the old "… load(s) total" summed legacy
  units) and the vehicle picker no longer falls back to "units". No
  "Refill Van" wording anywhere in app/.

  **Tests**: 31 new; 34 assertions across AC/AD/AE/AF/Z.2 that encoded
  the superseded manual-code rule were updated to the new rule (tests
  that only needed a record now seed a legacy-coded row directly). 3
  consecutive clean runs. Railway: migrate NO · seed NO · redeploy YES.

- **Milestone AG ("Internal Code Columns, Core Master Data Numbering &
  Dispatch Routing QA")** — migration YES (additive only).

  **Schema**: migration `drizzle/0019_loud_taskmaster.sql` — five
  `ADD COLUMN ... text` (all nullable), all default NULL: `customer_code`
  on `customers`; `site_code` on `customer_locations`; `vehicle_code` on
  `vehicles`; `driver_code` on `drivers`; `loading_point_code` on
  `warehouses`. Plus five new indexes: four tenant-scoped unique indexes
  (customers, vehicles, drivers, warehouses — Postgres permits multiple
  NULLs in a unique index, so legacy rows coexist freely with no
  backfill), and one plain index on `customer_locations.site_code`
  (that table has no tenant_id; per-tenant uniqueness is guaranteed
  upstream by `numbering_sequence_ledger`'s own unique constraint).
  Applied locally; all 17 existing customers, 14 vehicles, 12 drivers,
  5 warehouses, and 12 customer locations confirmed preserved with NULL
  codes. Zero destructive operations.

  **Numbering registry**: all five entities flipped from schema-gap →
  converted in `CODE_FIELD_ENTITY_MAP`. No entity remains schema-gap.

  **API conversion**: POST for Customers, Vehicles, Drivers, Loading
  Points (warehouses table), and Customer Sites (locations route, which
  resolves tenantId from the parent customer since customer_locations has
  no tenant_id). All five wired with `resolveEntityCode` + `linkLedgerToRecord`.
  PATCH immutability guards on Customers, Vehicles, and Loading Points
  (Drivers has no PATCH route). All accept only system-generated codes;
  any client-supplied code returns 400 MANUAL_CODE_REJECTED. Plate number
  and license number remain manual, required, and completely separate.

  **Existing records**: all keep NULL codes permanently (no backfill, no
  renumbering). `customerCode: true` added to SAFE_CUSTOMER_LIST_COLUMNS
  so the field appears in Customer API responses.

  **Test isolation**: `SERIES_PREFIX` and `ensureAllSeries()` extended to
  cover all 5 new entity types. `makeSupplierSeries` made fully idempotent
  for all entity types (was previously SUPPLIER-only). Seven AE test
  isolation failures diagnosed and fixed (entity types swapped to
  audit-only; deactivate/restore pattern for no-series tests).

  **Tests**: 30 new in `milestoneAGCodeColumns.test.ts` — schema
  columns, migration integrity, registry, API allocation, immutability,
  concurrency, tenant isolation, legacy NULL support, passwordHash
  exclusion, and protected-file verification. 1257/1257 across 91 files,
  three consecutive clean runs.

  No seedData, pricing, billing, ERP, dispatch-runtime or driver-app
  changes (file timestamps confirmed). Railway: migrate YES (additive) ·
  seed NO · wipe/reseed NO · redeploy YES.

- **Phase 1 RC1 ("Trip Lifecycle Events, Procurement Workflow, Apply
  Recommended Numbering, Dashboard & Reports Pages")** — migration YES
  (additive only).

  **Schema**: migration `drizzle/0020_fat_dorian_gray.sql` — one
  `CREATE TABLE "trip_lifecycle_events"` with columns: id, tenantId,
  tripId, eventType (text), actorUserId, driverId, lat/lng, notes,
  loadedLiters, deliveredLiters, stopId, createdAt. Three plain
  indexes. Zero destructive operations.

  **Trip Lifecycle Events** (`GET/POST /api/trips/[id]/lifecycle`):
  six-stage operational event log (STARTED, ARRIVED_LOADING,
  LOADING_COMPLETE, ARRIVED_SITE, UNLOADING_COMPLETE, CLOSED, NOTE).
  Built entirely around the existing Milestone W/P.2 billing/POD
  logic — the route never modifies `trip.status`, `stop.status`,
  invokes `autoCloseTripIfAllStopsResolved`, touches `epods`, or
  generates invoices. Driver page gains a `logLifecycleEvent` helper
  and a "Mark Trip Started" button (purely additive).

  **Procurement Workflow** — full PR→PO→GR pipeline where Z.1
  previously had read-only stubs: `POST /api/purchase-requisitions`
  (with lines, priority, justification), `PATCH [id]`
  (submit/approve/reject with ADMIN-only guard), `POST
  /api/purchase-orders` (with lines, pre-fills from approved PR),
  `POST /api/goods-receipts` (SQL-UPSERT inventory posting to
  `maintenance_inventory_balances` + movement audit row). Procurement
  UI rewritten with four tabs: Suppliers (NextCodePreview), PRs
  (create + submit/approve/reject inline), POs (create from approved
  PR), Goods Receipts (receive against PO with accepted-qty inventory
  posting confirmation).

  **Apply Recommended Numbering** (`GET/POST
  /api/settings/numbering-apply-recommended`): previews which of the
  12 standard Smarty1 series are missing for the tenant; explicit
  `confirm: true` creates missing ones, never overwrites existing.
  Settings page gains an "Apply Recommended Smarty1 Numbering" widget
  with preview → confirm flow.

  **New pages**: `app/admin/dashboard/page.tsx` (standalone Operations
  Dashboard at /admin/dashboard with KPI cards and quick-nav);
  `app/admin/reports/page.tsx` (standalone Reports at /admin/reports
  using the existing report engine with column picker + CSV export).

  **Tests**: 31 new in `milestoneRC1.test.ts`; 8 assertions across
  AA/AC/AF/AF.1/AF-conv/Z.2 updated to reflect the RC1 additions.
  1288/1288 across 92 files, three consecutive clean runs. No
  seedData, pricing, billing, ERP, dispatch-runtime or POD changes —
  confirmed by file timestamps. Railway: migrate YES (additive) ·
  seed NO · redeploy YES.

- **Phase 1 RC1 Closeout ("RBAC Phase 1, Period Reset, ERP Numbering
  Convergence, Inventory Actions, Driver Lifecycle Completion,
  Procurement UX")** — migration YES (additive).

  **Migration**: `drizzle/0021_regular_praxagora.sql` — four new tables
  (`roles`, `permissions`, `role_permissions`, `user_roles`) + one
  column (`expense_claims.expense_ref text`). 8 indexes. Zero
  destructive operations. Applied to dev and test DBs.

  **RBAC Phase 1** (`lib/rbac.ts`): 12 system roles (PLATFORM_ADMIN,
  TENANT_ADMIN, DISPATCHER, DISPATCH_SUPERVISOR, FLEET_MANAGER,
  DRIVER, MAINTENANCE_MANAGER, MAINTENANCE_TECHNICIAN, PROCUREMENT,
  INVENTORY, FINANCE, VIEWER) seeded lazily via `ensureSystemRoles()`.
  `getUserModules()` backward-compatible with legacy `user.role`
  strings. `GET/POST/DELETE /api/roles` and `/api/user-roles` routes.
  Settings Users & Roles section shows all tenant users with
  assign/revoke role controls.

  **Allocator period reset**: `allocateNextNumber()` now resets
  `nextNumber` to 1 when the current `periodKey` differs from the
  last ledger row for YEARLY/MONTHLY series. Checked and reset
  atomically within the same database transaction.

  **ERP numbering convergence**: contracts use `resolveEntityCode`
  with `genNumber("CNT")` fallback; expenses write to the new
  `expenseRef` column via EXPENSE series; PR/PO/GR all use
  `resolveEntityCode` with timestamp fallbacks. Every entity class
  that previously used timestamp-based IDs now accepts ERP-style
  numbers from the allocator when a series is configured.

  **Inventory adjustment**: `POST /api/maintenance-inventory/adjust`
  — handles ADJUSTMENT, MAINTENANCE_ISSUE, TRANSFER_OUT, TRANSFER_IN,
  RETURN; SQL-UPSERTs `maintenance_inventory_balances`; blocks
  negative movements that exceed available stock. Inventory UI
  shows real `AdjustmentForm` replacing the "later milestones"
  placeholder.

  **Driver lifecycle completion**: all 6 stages (STARTED,
  ARRIVED_LOADING, LOADING_COMPLETE, ARRIVED_SITE, UNLOADING_COMPLETE,
  CLOSED) exposed as individual log buttons in the driver app. Still
  additive to existing W/P.2 billing/POD logic.

  **Procurement UX**: browser `prompt()` replaced with inline
  `RejectButton` component with text input and confirm/cancel.

  **Tests**: 30 new (`milestoneRC1Closeout.test.ts`). 10 existing
  assertions updated. 1318/1318 across 93 files, three consecutive
  clean runs. Railway: migrate YES · seed NO · redeploy YES.
