...
const rejectSchema = z.object({ reviewNotes: z.string().min(1) });
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const _permDeny1 = await checkPermission(session, tenantId, PERMISSIONS.EXPENSES_APPROVE); if (_permDeny1) return _permDeny1;
  ...
  if (claim.status !== "PENDING") {
    return NextResponse.json({ error: "Only a PENDING claim can be rejected" }, { status: 422 });
  }
  ...
  await db
    ...
    .set({ status: "REJECTED", reviewedByUserId: userId, reviewedAt: new Date(), reviewNotes: parsed.data.reviewNotes })
