...
export async function POST(req: NextRequest) {
  ...
  const id = genId();
  // RC1: EXPENSE requires a configured numbering series — no timestamp fallback.
  let expenseRef: string;
  try {
    const _r = await resolveEntityCode({ tenantId, entityType: "EXPENSE", entityLabel: "expense", codeLabel: "expense number", provided: undefined });
    if (!_r.ok) return NextResponse.json({ error: "CONFIGURE_NUMBERING", message: "No active EXPENSE numbering series. Go to Settings → Numbering and apply recommended series." }, { status: 422 });
    expenseRef = _r.code;
  } catch (err: any) {
    if (err?.message?.includes("No active")) return NextResponse.json({ error: "CONFIGURE_NUMBERING", message: "No active EXPENSE numbering series. Go to Settings → Numbering and apply recommended series." }, { status: 422 });
    throw err;
  }
  await db.insert(expenseClaims).values({ id, tenantId, expenseRef, ...data });
  ...
  const created = await db.query.expenseClaims.findFirst({
    ...
    with: { driver: { with: { user: { columns: SAFE_USER_COLUMNS } } }, vehicle: true },
  });
