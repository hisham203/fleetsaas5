...
export async function POST(req: NextRequest) {
  ...
  const existing = await db.query.itemSubcategories.findFirst({ where: and(eq(itemSubcategories.tenantId, tenantId), eq(itemSubcategories.code, data.code)) });
  if (existing) {
  ...
  await db.insert(itemSubcategories).values({
    id, tenantId, categoryId: data.categoryId, code: data.code, name: data.name, description: data.description, status: data.status ?? "ACTIVE",
  });
