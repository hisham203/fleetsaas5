...
// Milestone Z.1 — schema-only foundation. Read-only. No PR
// creation/approval workflow exists yet.
export async function GET(req: NextRequest) {
  ...
  const rows = await db.query.purchaseRequisitions.findMany({ where: and(...conditions) });
