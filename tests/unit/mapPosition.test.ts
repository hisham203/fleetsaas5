...
describe("resolveTripMapPosition (Live Dispatch Map marker fallback)", () => {
  it("prefers the live GPS position when both current and fallback coordinates exist", () => {
    const result = resolveTripMapPosition(24.71, 46.67, 24.80, 46.60);
    expect(result).toEqual({ lat: 24.71, lng: 46.67 });
  });

  it("falls back to the first stop's location when no GPS ping has landed yet", () => {
    const result = resolveTripMapPosition(null, null, 24.80, 46.60);
    expect(result).toEqual({ lat: 24.80, lng: 46.60 });
  });
  ...
  it("treats a real 0,0 coordinate as valid rather than falsy (0 is a legitimate latitude/longitude)", () => {
